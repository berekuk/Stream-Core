...
=head1 SYNOPSIS
    ...
    use Streams;
    process catalog->in('links') => processor(sub {
        print "Link: ".shift;
    });
...
Filter is a kind of output stream which returns transformed data further.

Filters can be attached to input or output streams to modify their behavior.
...
This is still a very early draft. API is still very unstable. Use with caution.
